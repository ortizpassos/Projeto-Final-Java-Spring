...
// Adicione o parâmetro use_gradient (padrão = false para manter compatibilidade)
lv_obj_t * new_screen(lv_obj_t * parent, bool use_gradient = false) {
  ...
  if (use_gradient) {
    ...
    // Fundo transparente (comportamento original)
...
void go_cont(){
     cont++;
       if (lbl_header != NULL) {  ;
   //lv_label_set_text_fmt(lbl_header, "Progresso: %d/%d", cont, cont_max);
...
void go_bl() {
  ...
  if (!scr_bl) {
    ...
    // lv_obj_set_style_bg_opa(scr_bl, LV_OPA_COVER, 0);
    // lv_obj_set_scroll_dir(scr_bl, LV_DIR_NONE);
    // lv_obj_set_style_bg_color(scr_bl, lv_color_hex(0x1A3D6B), 0);
    // lv_obj_set_style_bg_grad_color(scr_bl, lv_color_hex(0xEA824D), 0);
    // lv_obj_set_style_bg_grad_dir(scr_bl, LV_GRAD_DIR_VER, 0);
...
void go_touch() {
  ...
  if (!scr_touch) {
    ...
    scr_touch = new_screen(NULL, true);
